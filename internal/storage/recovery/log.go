...
func NewWriteAheadLog(disk DiskAdapter, fileName string) (*WriteAheadLog, error) {
	if disk == nil {
		return nil, errors.New("disk adapter is required")
	}
...
func (w *WriteAheadLog) AppendPageWrite(page *disk.Page, offset int, payload []byte) (LogRecord, error) {
	...
	if _, err := w.diskAdapter.AppendFile(w.fileName, encoded); err != nil {
		w.mu.Unlock()
		return LogRecord{}, err
	}
...
func (w *WriteAheadLog) Flush() error {
	...
	if err := w.diskAdapter.SyncFile(w.fileName); err != nil {
		return err
	}
...
// Records reads all log records from the WAL file in order.
// It returns an error if any record is invalid or if there is an I/O error.
func (w *WriteAheadLog) Records() ([]LogRecord, error) {
	size, err := w.diskAdapter.FileSize(w.fileName)
	if err != nil {
		return nil, err
	}
	if size == 0 {
	...
	for offset := int64(0); offset < size; {
		...
		n, err := w.diskAdapter.ReadFileAt(w.fileName, offset, lengthBuf)
		if err != nil {
		...
		n, err = w.diskAdapter.ReadFileAt(w.fileName, offset+walLengthFieldSize, body)
...
func decodeLogRecord(body []byte) (LogRecord, error) {
	...
	expectedChecksum := config.ByteOrder.Uint32(body[:walChecksumFieldSize])
	...
	if err := binary.Read(reader, config.ByteOrder, &record.LSN); err != nil {
		return LogRecord{}, err
	}
	...
	var pageNum uint64
	if err := binary.Read(reader, config.ByteOrder, &pageNum); err != nil {
		return LogRecord{}, err
	}
	...
	if err := binary.Read(reader, config.ByteOrder, &record.Offset); err != nil {
		return LogRecord{}, err
	}

	var payloadLen uint32
	if err := binary.Read(reader, config.ByteOrder, &payloadLen); err != nil {
		return LogRecord{}, err
	}
